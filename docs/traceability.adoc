...
| CON-06 (Integración con pasarela de pagos externa)
| Uso de patrón Adapter y API segura para aislar proveedor.
| Servicio de Pagos, Componente PasarelaDePagos.
| Cumple

| CON-07 (Arquitectura Cliente-Servidor)
| Separación física y lógica de UI y Backend.
| Vista Lógica 7.5, Vista de Despliegue 7.8.
| Cumple

| CON-08 (Modelo en capas)
| Organización estricta: Presentación -> Negocio -> Datos.
| Vista Lógica 7.5, Estructura de paquetes Backend.
| Cumple

| CON-09 (Comunicación segura)
| Enforce de HTTPS y TLS 1.3 en Gateway y Nginx.
| Vista de Despliegue 7.8 (Balanceador de Carga).
| Cumple

| CON-10 (Registro obligatorio de transacciones)
| Logging transaccional síncrono en base de datos.
| Módulo de Auditoría, Entidad AuditoriaInfo.
| Cumple

| CON-11 (Multiplataforma web y móvil)
| API RESTful agnóstica consumida por SPA y Apps.
| Capa de Cliente (Vista 7.5), Controladores REST.
| Cumple

| CON-12 (Desacoplamiento entre cliente y servidor)
| API Contract First, DTOs para transferencia de datos.
| Interfaces de Servicio, Controladores REST.
| Cumple

| CON-13 (Rendimiento mínimo garantizado)
| Optimización de queries, índices y caché distribuido.
| Vista de Implementación (Persistencia), Redis.
| Cumple

| CON-14 (Política de recuperación ante desastres)
| Replicación de BD Maestro-Esclavo y Backups programados.
| Vista de Despliegue 7.8 (Nodos de BD).
| Cumple

| CON-15 (Despliegue modular y escalable)
| Contenedores independientes para cada capa/servicio.
| Dockerfile por servicio, docker-compose/K8s.
| Cumple
...
| CRN-02 (Experiencia fluida del usuario)
| SPA con gestión de estado local y carga asíncrona.
| Capa de Cliente (App Web/Móvil), React/Angular.
| Cumple

| CRN-03 (Seguridad de datos y autenticación)
| Centralización en Identity Provider + cifrado en reposo.
| Módulo de Autenticación, Base de Datos Segura.
| Cumple

| CRN-04 (Escalabilidad progresiva)
| Diseño stateless en servicios para escalado horizontal.
| Vista de Despliegue (Cluster K8s/Docker).
| Cumple

| CRN-05 (Mantenibilidad por capas)
| Inyección de Dependencias para desacoplar capas.
| Configuración de DI, Interfaces de Servicios.
| Cumple

| CRN-06 (Eficiencia en la comunicación cliente-servidor)
| Payloads JSON compactos, compresión GZIP, paginación.
| API Gateway, DTOs optimizados.
| Cumple

| CRN-07 (Disponibilidad continua)
| Health Checks, probes de Kubernetes y reinicio automático.
| Configuración de Despliegue, Endpoint /healthz.
| Cumple

| CRN-08 (Observabilidad y monitoreo)
| Logs estructurados (Serilog), Métricas (Prometheus).
| Stack de Observabilidad, Middleware de Logging.
| Cumple

| CRN-09 (Integración con servicios externos)
| Facades y Adapters para encapsular terceros.
| Componentes de Integración (Email, Pagos).
| Cumple

| CRN-10 (Consistencia e integridad de datos)
| Transacciones ACID para operaciones críticas.
| Gestor de Reservas (Transaction Scope).
| Cumple

| CRN-11 (Recuperación ante fallos)
| Circuit Breaker (Polly) y Retries automáticos.
| Clientes HTTP resilientes, Vista Proceso 8.3.
| Cumple

| CRN-12 (Despliegue controlado)
| Migraciones de BD versionadas y Pipelines CI/CD.
| Scripts de Migración, Configuración de Pipeline.
| Cumple

| CRN-13 (Separación de responsabilidades)
| Definición estricta de límites de contexto (Bounded Contexts).
| Estructura de Solución, Namespaces.
| Cumple

| CRN-14 (Compatibilidad con dispositivos móviles)
| Diseño Responsive y API única para todos los clientes.
| Frontend CSS/Framework, API REST unificada.
| Cumple

| CRN-15 (Evolución tecnológica)
| Uso de interfaces estándar para facilitar reemplazo.
...
| Consultas de lectura optimizadas + DTOs de proyección.
| Servicio Reservaciones, Vista Lógica 7.5.
| Cumple

| CU-05 (Seleccionar servicios adicionales en reservación)
| Patrón Decorator o agregación en entidad Reservación + Actualización transaccional.
| Servicio Reservaciones, Servicio Catálogo, Vista 7.5.
| Cumple

| CU-06 (Consultar historial de reservaciones)
| Filtrado en repositorio + Paginación en API.
| Servicio Historial/Reservas, Repositorio Reservas.
| Cumple

| CU-07 (Gestionar cuenta de usuario)
| CRUD estándar + Validación de identidad (Identity Management).
| Servicio Usuarios, Vista de Implementación 7.6.
| Cumple

| CU-08 (Guardar métodos de pago)
| Tokenización segura (PCI-DSS compliance) + Integración externa.
| Servicio Pagos, Integración Pasarela.
| Cumple

| CU-09 (Dejar reseña)
| Asociación de entidad Reseña con Reservación + Validación de estado "Finalizada".
| Servicio Reseñas, Repositorio Reseñas.
| Cumple
...
| Reutilización de lógica de negocio (Service Layer) con permisos de Recepcionista.
| Servicio Reservaciones, Servicio Disponibilidad.
| Cumple

| CU-13 (Cancelar reservación en recepción)
| Extensión de lógica de cancelación + Auditoría de usuario interno.
| Servicio Cancelación, Módulo Auditoría.
| Cumple

| CU-14 (Cambiar a huésped de habitación)
| Transacción atómica (Liberar actual + Ocupar nueva) + Ajuste de cargos.
| Servicio Estancias, Servicio Disponibilidad.
| Cumple

| CU-15 (Registrar consumos y servicios)
| Asociación de consumos a la cuenta de la habitación (Folio).
| Servicio Consumos, Entidad Estancia.
| Cumple

| CU-16 (Consultar estado de reservación en recepción)
| Visualización detallada para rol administrativo + Consultas Join.
| Servicio Recepción, Vista de UI Recepcionista.
| Cumple

| CU-17 (Extender estancia del huésped)
| Validación de disponibilidad futura + Recálculo de tarifa.
| Servicio Estancias, Motor de Precios.
| Cumple

| CU-18 (Consultar disponibilidad local)
| Misma lógica de CU-01 pero sin caché agresivo (datos en tiempo real).
| Servicio Disponibilidad, Acceso directo a BD.
| Cumple

| CU-19 (Reportar incidencia)
| Registro de tickets + Notificación asíncrona a mantenimiento.
| Gestor de Incidentes, Servicio Notificaciones.
| Cumple

| CU-20 (Añadir hotel al catálogo)
| CRUD Administrativo + Validación de integridad referencial.
| Gestor de Catálogo, Repositorio Hoteles.
| Cumple

| CU-21 (Editar hotel)
| Actualización de entidad + Invalidación de caché de disponibilidad.
| Gestor de Catálogo, Servicio Caché.
| Cumple

| CU-22 (Crear tipo de habitación)
| Definición de metadatos y capacidades en catálogo.
| Gestor de Catálogo, Entidad TipoHabitación.
| Cumple

| CU-23 (Actualizar tipo de habitación)
| Modificación de atributos + Control de impacto en reservas futuras.
| Gestor de Catálogo.
| Cumple

| CU-24 (Registrar habitación)
| Alta de inventario físico + Asignación de estado inicial.
| Gestor de Inventario, Repositorio Habitaciones.
| Cumple

| CU-25 (Modificar habitación)
| Gestión de estados operativos (Mantenimiento, Limpieza).
| Gestor de Inventario.
| Cumple

| CU-26 (Administrar precios diarios)
| Estrategia de precios (Revenue Management) + Actualización masiva.
| Servicio Tarifas, Base de Datos.
| Cumple

| CU-27 (Agregar consumo al catálogo)
| Gestión de inventario de productos/servicios vendibles.
| Gestor de Catálogo.
| Cumple

| CU-28 (Agregar servicio al catálogo)
| Definición de servicios intangibles y costos base.
| Gestor de Catálogo.
| Cumple

| CU-29 (Editar consumo)
| Actualización de precios y descripciones en catálogo.
| Gestor de Catálogo.
| Cumple

| CU-30 (Editar servicio)
| Actualización de parámetros de servicio.
| Gestor de Catálogo.
| Cumple

| CU-31 (Generar reportes financieros)
| Agregación de datos (Data Warehouse o consultas pesadas) + Exportación.
| Servicio Reportes, Vista Proceso.
| Cumple

| CU-32 (Auditar cancelaciones y reembolsos)
| Consulta de logs de auditoría filtrados por tipo de evento.
| Módulo Auditoría, Vista Lógica 7.5.
| Cumple

| CU-33 (Consultar historial de reservaciones)
| Vista global de reservas sin restricciones de usuario.
| Servicio Reportes, Repositorio Reservas.
| Cumple

| CU-34 (Ver logs de acceso al sistema)
| Consulta a repositorio de logs de seguridad/acceso.
| Módulo Auditoría, Servicio Seguridad.
| Cumple

| CU-35 (Generar reportes de cumplimiento)
| Verificación de reglas de negocio contra registros históricos.
| Servicio Reportes, Motor de Reglas.
| Cumple

| CU-36 (Crear perfil de administrador)
| Alta de usuario con rol privilegiado + Auditoría estricta.
| Servicio Usuarios, RBAC.
| Cumple

| CU-37 (Editar perfil de administrador)
| Gestión de credenciales y permisos.
| Servicio Usuarios, Módulo Auth.
| Cumple

| CU-38 (Eliminar perfil de administrador)
| Baja lógica (Soft Delete) para mantener integridad referencial.
| Servicio Usuarios.
| Cumple

| CU-39 (Crear perfil de recepcionista)
| Alta de usuario operativo con restricciones de acceso.
| Servicio Usuarios, RBAC.
| Cumple
...
| Baja lógica de usuario operativo.
| Servicio Usuarios.
| Cumple
