...
2. El huésped da clic en "Sí" y termina el caso de uso. +
3. El huésped da clic en "No" y regresa al paso anterior del flujo normal. +
*FA-02:* Clic en "Cancelar proceso": +
1. El sistema regresa al paso 1 del flujo normal.
| **Excepciones** |
*EX-01:* Error de base de datos: +
1. El sistema muestra un mensaje indicando que hubo un error técnico. +
2. El huésped da clic en "Aceptar" y termina el caso de uso.
...
| **Flujo normal** |
1. El sistema muestra una tabla con las reservaciones del huésped (fecha de inicio, fecha fin, hotel, consumos) ordenadas por fecha. **(Ver FA-01)** **(Ver EX-01)** +
...
2. El huésped da clic en "Aceptar" y termina el caso de uso.
| **Postcondiciones** |
*POST-01:* Huésped obtiene datos de reservaciones anteriores.
| **Reglas de negocio** | Ninguna
| **Incluye** | Ninguna
| **Extiende** | Nunguna
|===
...
| **Precondiciones** |
| **Flujo normal** |
1. El sistema muestra los datos del huésped (nombre, apellido paterno, apellido materno, número telefónico, correo electrónico, rfc) **(Ver EX-01)** +
2. El huésped ingresa los datos en los campos que desea modificar y da clic en “Guardar” **(Ver FA-01)** **(Ver FA-02)** **(Ver FA-03)** +
3.	El sistema guarda la modificación de los datos del huésped y muestra un mensaje de guardado exitoso **(Ver EX-01)** +
...
| **Precondiciones** |
| **Flujo normal** |
1. El sistema muestra los métodos de pago registrados y  los campos para agregar un nuevo método de pago (Nombre, número de cuenta, CVV, fecha de expiración). **(Ver EX-01)** +
2. El huésped agrega los campos y da clic en “Guardar”. **(Ver FA-01)** **(Ver FA-02)** **(Ver FA-03)** **(Ver FA-04)** **(Ver FA-05)** +
...
2.	El huésped da clic en “Aceptar” +
3.	El sistema regresa al paso 1 del flujo normal +
*FA-04:* Fecha de método de pago expirada +
1.	El sistema muestra un mensaje de fecha de tarjeta invalida. +
2.	El huésped da clic en “Aceptar”. +
3.	El sistema regresa al paso del 1 del flujo normal. +
*FA-05:* Método de pago inválido +
...
| **Flujo normal** |
1. El sistema muestra los datos de la reservación (fechas, tipo de habitación, estado). **(Ver EX-01)**
...
| **Precondiciones** |
| **Flujo normal** |
1. El sistema carga y muestra los datos del hotel local (numero de hotel) y muestra un formulario de quejas (titulo de incidente, fecha, descripción, área de incidente). **(Ver EX-01)**
2. El recepcionista ingresa los datos de incidente y da clic en "Reportar". **(Ver FA-01)** **(Ver FA-02)**
3. El sistema valida los datos y guarda el registro de incidente en la base de datos.
| **Flujos alternos** |
*FA-01:* Clic en "Cancelar": +
1. El sistema muestra un mensaje de confirmación para cancelar el proceso. +
2. El recepcionista da clic en "Sí" y termina el caso de uso.
...
| **Reglas de negocio** | RN-01 Fecha de reporte de incidente no puede ser reportada un mes después del incidente.
| **Incluye** | Ninguna
| **Extiende** | Ninguna
|===
...
5. El sistema muestra los campos a cambiar de la habitación (número de habitación, tipo de habitación, características iniciales y estado). **(Ver EX-01)**
...
| **Flujo normal** |
1.	El sistema carga en una tabla los consumos registrados en el catálogo de la cadena y los campos para agregar un consumo (nombre, descripción, precio). *(Ver EX-01)*  +
2.	El administrador ingresa los datos del nuevo consumo y da clic en “Aceptar”. *(Ver FA-01)* *(Ver FA-02)* +
3.	El sistema guarda el consumo en la base de datos y muestra el mensaje de registro exitoso. *(Ver EX-01)*
| **Flujos alternos** |
*FA-01:* Clic en "Cancelar" +
1. El sistema muestra un mensaje de confirmación para cancelar el proceso. +
2. El huésped da clic en "Sí" y termina el caso de uso. +
3. El huésped da clic en "No" y regresa al paso anterior del flujo normal. +
*FA-02:* Consumo ya existente en el sistema +
...
| **Flujo normal** |
1. El sistema muestra en una tabla los datos de cancelaciones y en otra los datos de reembolsos ordenados por fecha.  **(Ver FA-01)** **(Ver EX-01)** +
...
| **Flujo normal** |
1. El sistema muestra un formulario de selección de estado y municipio. **(Ver EX-01)** +
2. El auditor selecciona un estado y municipio. +
3. El sistema muestra una tabla de los hoteles. **(Ver EX-01)** +
